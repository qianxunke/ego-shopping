...
func (s *service) createTokenClaims(subject *Subject) (m *jwt.StandardClaims, err error) {
	...
	m = &jwt.StandardClaims{
		ExpiresAt: nowTime.Add(tokenExpiredDate).Unix(),
		NotBefore: nowTime.Unix(),
		Id:        subject.ID,
		IssuedAt:  nowTime.Unix(),
		Issuer:    "shop.qianxunke.com",
		Subject:   subject.ID,
	}
	return
}
...
func (s *service) saveTokenToCache(subject *Subject, val string) (err error) {
	if err = clients.GetRedis().Set(tokenIDKeyPrefix+subject.ID, val, tokenExpiredDate).Err(); err != nil {
...
func (s *service) delTokenFromCache(subject *Subject) (err error) {
	//保存
	if err = clients.GetRedis().Del(tokenIDKeyPrefix + subject.ID).Err(); err != nil {
...
func (s *service) getTokenFromCache(subject *Subject) (token string, err error) {
	// 获取
	tokenCached, err := clients.GetRedis().Get(tokenIDKeyPrefix + subject.ID).Result()
	if err != nil {
