...
import (
	...
	consulLock "github.com/micro/go-micro/sync/lock"
	"reflect"
)
...
func (s *service) GetSellSkuStock(ctx context.Context,req *productSkuStockProto.In_GetSellSkuStock) (rsp *productSkuStockProto.Out_GetSellSkuStock,err error) {
	...
	//开始锁库
	err = lock.Acquire(id, consulLock.TTL(time.Duration(time.Second*3)), consulLock.Wait(time.Duration(time.Second*3)))
	if err != nil {
		rsp.Error = &productSkuStockProto.Error{
			Code:    http.StatusInternalServerError,
			Message: err.Error(),
